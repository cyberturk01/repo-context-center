...
| --- | ---: | --- |
| `src/cli/verify/buildVerify.ts`, `tests/verify.test.js` | 26 | Polish verify build alignment and translation domain calibration |
...
| Scope | Command | Count |
| --- | --- | ---: |
| build | `npm run build` | 107 |
| tests | `npm test` | 48 |
| impact | `node --test tests/impact.test.js` | 26 |
| verify | `node --test tests/verify.test.js` | 25 |
| cli | `node --test tests/cli.test.js` | 22 |
| outputcontract | `node --test tests/outputContract.test.js` | 17 |
| work | `node --test tests/work.test.js` | 15 |
| handoff | `node --test tests/handoff.test.js` | 12 |

## Repository Habits

- Completed work records meaningful changed files (163/167).
- Verification commands are recorded with completed work (167/167).
- Tests are commonly changed with related implementation work (145/167).
- Follow-ups are captured when residual tasks remain (1/167).

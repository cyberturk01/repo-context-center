...
| Generated/noise filtering | Active (2 ignored/noise areas separated) |
...
| Tests / Fixtures: `tests/fixtures/learning-cases.json`, `tests/fixtures/routing-cases.json` | Fixture changes can make tests pass while real map output gets worse. | npm run build |
| Staff/POS/public flows: `src/core/renderRepositoryLearning.ts`, `src/core/repositoryLearning.ts`, `src/core/repositoryLearningRouting.ts` | Public and staff flows are user-visible and often role-sensitive. | npm run build |
...
| Release workflow: `.github/workflows/ai-project-guardian.yml`, `.github/workflows/ci.yml`, `scripts/release-check.js` | Workflow changes can block releases or deploy broken builds. | npm run build |
| Fixture/snapshot drift: `tests/fixtures/learning-cases.json`, `tests/fixtures/routing-cases.json` | Fixtures and expected output can drift from generated map behavior. | npm run build |

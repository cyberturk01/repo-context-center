...
| --- | --- |
| CLI behavior | `src/cli/index.ts`, `src/cli/commands/archive.ts`, `src/cli/commands/decision.ts`, `src/cli/commands/done.ts` |
...
| Tests / fixtures | `tests/agent-startup-adoption.test.js`, `tests/archive.test.js`, `tests/cli.test.js`, `tests/decision.test.js` |

### Task Routing
| Task Type | Start With | Then Check | Tests | Notes |
| --- | --- | --- | --- | --- |
| CLI flags/output | `src/cli/commands/archive.ts`, `src/cli/commands/decision.ts`, `src/cli/commands/done.ts`, `src/cli/commands/estimate.ts` | core command handler, README examples, CLI tests | `tests/archive.test.js`, `tests/cli.test.js`, `tests/decision.test.js` | Keep output stable for tests and scripts. |
...
| GitHub Actions / release workflow | `.github/workflows/ai-project-guardian.yml`, `.github/workflows/ci.yml` | package scripts, workflow files, release docs | `tests/v07-release.test.js` | Use Investigation Mode before changing deploy or release behavior. |

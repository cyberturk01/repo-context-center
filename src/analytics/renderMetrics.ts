...
export function renderMetricsText(metrics: RepositoryMetrics): string {
  return `${[
    ...
    "",
    "Ecosystem:",
    `- Primary: ${metrics.ecosystem.primary}`,
    `- Confidence: ${metrics.ecosystem.confidence}`,
    `- Detected ecosystems: ${formatNumber(metrics.ecosystem.detected)}`,
    `- Matched signals: ${formatNumber(metrics.ecosystem.signals)}`,
    `- Package root: ${metrics.ecosystem.packageRoot ?? "unknown"}`,
    `- Monorepo: ${metrics.ecosystem.monorepo ? "yes" : "no"}`,
    `- Workspace: ${metrics.ecosystem.workspaceDetected ? metrics.ecosystem.workspaceType : "none"}`,
    `- Workspace packages: ${formatNumber(metrics.ecosystem.workspacePackages)}`,
    `- Package scope: ${metrics.ecosystem.packageScope ?? "unknown"}`,
    "",
    "Routing:",
